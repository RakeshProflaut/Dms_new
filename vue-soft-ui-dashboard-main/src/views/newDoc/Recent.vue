...
export default {
  ...
  computed: {
    displayedData() {
      ...
      return this.recentDocDetails.slice(start, end)
    },
    pages() {
      return Math.ceil(this.recentDocDetails.length / this.itemsPerPage)
    },
  },
  methods: {
    async getRecentData() {
      const apiUrl =
        this.selectedFilter === 'Folders'
          ? 'http://localhost:61050/dms/home/getAllRecentFolders'
          : 'http://localhost:61050/dms/home/getAllRecentFiles'
    ...
    filterDisplayedData(query) {
      ...
      this.recentDocDetails = this.recentDocDetails.filter((item) =>
    ...
    updateDisplayedData() {
      ...
      const start = (this.page - 1) * this.itemsPerPage
      const end = start + this.itemsPerPage
      this.displayedData = this.recentDocDetails.slice(start, end)
    },
  },
}
</script>
